...
const codePromise = new Promise((resolveCode, reject) => {
  server.once('request', (request, response) => {
    ...
    response.writeHead(error || !code || state !== oauthState ? 400 : 200, {
      'content-type': 'text/plain; charset=utf-8'
    });
    response.end(
      error || !code || state !== oauthState
    ...
    else if (state !== oauthState) reject(new Error('OAuth state mismatch.'));
    else resolveCode(code);
  });
});

console.log(`Opening Google authorization:\n${authorizationUrl}\n`);
console.log('Sign in with the Google account that administers manager 925-809-1560.');
...
try {
  ...
  if (!tokens.refresh_token) {
    throw new Error(
      'Google did not return a refresh token. Revoke the app grant at https://myaccount.google.com/permissions and run authorization again.'
    );
  }
