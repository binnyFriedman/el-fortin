...
function readDeveloperToken() {
  const attempts = [];
  if (process.env.USER) {
    attempts.push(['-a', process.env.USER, '-s', KEYCHAIN_SERVICE, '-w']);
  }
  attempts.push(['-s', KEYCHAIN_SERVICE, '-w']);

  for (const args of attempts) {
    try {
      const token = execFileSync('security', ['find-generic-password', ...args], {
        encoding: 'utf8',
  ...
  throw new Error('Google Ads developer token is missing from Keychain.');
}
